...
Next, add the bot to your server with `messages.read` scope (and `Message Content` intenet.) As well as `Manage Messages`  and `Send Messages` permission (so it can delete the bad url)
